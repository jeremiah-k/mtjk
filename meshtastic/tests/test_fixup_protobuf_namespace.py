...
rewrite_proto_source = _fixup.rewrite_proto_source
rewrite_proto_file = _fixup.rewrite_proto_file
rewrite_proto_directory = _fixup.rewrite_proto_directory
...
@pytest.mark.unit
def test_rewrite_proto_source_is_generic_for_future_custom_options():
    source = "uint32 value = 1 [(meshtastic.future_option) = true];\n"

    assert rewrite_proto_source(source) == (
...
def test_rewrite_proto_source_preserves_option_like_text_in_strings_and_comments():
    ...
    assert "(meshtastic.comment_option)" in rewritten
    assert "(meshtastic.block_option)" in rewritten
    assert '"(meshtastic.string_option)"' in rewritten
...
def test_rewrite_proto_source_is_idempotent():
    ...
    assert rewrite_proto_source(once) == once
    assert "meshtastic.protobuf.protobuf" not in once
    assert "meshtastic/protobuf/protobuf" not in once


@pytest.mark.unit
def test_rewrite_proto_file_reports_whether_content_changed(tmp_path: Path):
    proto = tmp_path / "config.proto"
    proto.write_text("package meshtastic;\n", encoding="utf-8")

    assert rewrite_proto_file(proto) is True
    assert rewrite_proto_file(proto) is False
...
@pytest.mark.unit
def test_rewrite_proto_directory_rejects_missing_directory(tmp_path: Path):
    missing = tmp_path / "missing"

    with pytest.raises(NotADirectoryError, match="missing"):
        rewrite_proto_directory(missing)
