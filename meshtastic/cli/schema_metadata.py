...
def _normalize_metadata(metadata: field_metadata_pb2.FieldMetadata) -> SchemaMetadata:
    ...
    return SchemaMetadata(
        ...
        deprecated=(metadata.deprecated if metadata.HasField("deprecated") else None),
...
def get_field_metadata(field: FieldDescriptor) -> SchemaMetadata | None:
    """Return Meshtastic metadata attached to ``field``, if present."""
    options = field.GetOptions()
    if not options.HasExtension(field_metadata_pb2.field_metadata):
        return None
    return _normalize_metadata(options.Extensions[field_metadata_pb2.field_metadata])


def get_enum_value_metadata(value: EnumValueDescriptor) -> SchemaMetadata | None:
    """Return Meshtastic metadata attached to an enum value, if present."""
